...
**推进原则（2026-09-07，Aaron："正常电脑推进剧情不会是因为点击了什么"）**
- 没有任何"点了什么才解锁"的门槛：没有灰态锁住的文件、没有步骤清单、没有"去看微信"的路标 toast、没有发光亮点。
- 剧情只靠两样东西往前走：① 时间（对方过一会儿才回、软件过一会儿才派单）；② 玩家发出去的话 / 在 ARGUS 里按的按钮。
  唯一的例外是 ARGUS 自己"看到"了什么（它是监控软件）：接入她电脑满 4 分钟或点开旅馆邮件后 20 秒，它报"已确认位置"。
- 学 ningning：微信随便发（自由输入原样发出去）、搜微信号 → 资料页 → 添加到通讯录才有会话（林晚 wanwan0315 / 孙宁 sunning_maple）、
  记事本可编辑且改动保存、真实的微信/小红书图标、通话记录黑字。
- 测试者看 [通关攻略.md](通关攻略.md)。
...
- 提示只有三种：微信新消息弹窗、ARGUS 图标闪烁/红点、音效。

## 设计规则（同步写在 state.js 头部注释）

1. 黑客解释必须始终成立。2. 软件来历只以三个互相矛盾的来源出现。3. 死亡不演，只在新闻页揭晓。
4. 前十分钟一条直线；不设门槛；软件派单就是教程。5. 不出现真实黑客工具名与手法。6. 页面不写死任何文字。
