/* =====================================================================
   聊天：自己的会话（own）+ 目标电脑上的微信（只读）。
   UI 复用微信 PC 壳（css/apps.css .wc-* / .wcm-*）。数据 = DEVICES[source].chats。
   ===================================================================== */
(function () {
    ...
    var current = null, readSet = {};

    function chats() {
        return ((DB.DEVICES[STATE.source()] || { chats: [] }).chats || []).filter(function (c) { return STATE.cond(c.visible); });
    }
    function byId(id) { return chats().filter(function (c) { return c.id === id; })[0] || null; }
    ...
    function renderList() {
        ...
        listEl.innerHTML = cs.map(function (c) {
            ...
            return (
                ...
                '<div class="i-main"><div class="i-name"><span>' + esc(T(c.nameRef)) + "</span></div>" +
    ...
    function renderPanel() {
        ...
        body.innerHTML = c.messages.map(function (m) {
            ...
            if (m.sys) return html + '<div class="wc-sys">' + esc(T(m.ref)) + "</div>";
            var me = m.from === "me";
            return html +
        ...
        foot.hidden = false;
        $("#wc-input").textContent = c.mode === "monitor" ? T("chat.monitor.input") : T("chat.input");
        body.scrollTop = body.scrollHeight;
    }

    function openChat(id) {
        current = id; readSet[id] = 1;
        renderList(); renderPanel();
        var c = byId(id);
        if (c) { if (c.sets) STATE.set(c.sets); STATE.emit("open-chat:" + id); }
    }
    function rerender() { renderList(); renderPanel(); }
    document.addEventListener("DOMContentLoaded", rerender);
    STATE.on(rerender);
    document.addEventListener("source-change", function () { current = null; readSet = {}; rerender(); });
})();
